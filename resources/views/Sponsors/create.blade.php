        ...
        <div class="col-12">
          ...
          <div class="card">
            ...
            <div class="form-group">
              <div class="row">
                <div class="col-md-6">
                  <label for="PaymentTerm">Periodic payments</label>
                  <select class="form-control"   name="PaymentTerm" id="PaymentTerm">
                  <option value="Once" 
                  @if(isset($sponsor) && $sponsor->PaymentTerm == 'Once')
                  selected
                  @endif
                  >Once</option>
                  <option value="Daily" 
                  @if(isset($sponsor) && $sponsor->PaymentTerm == 'Daily')
                  selected
                  @endif
                  >Daily</option>
                  <option value="Monthly" 
                  @if(isset($sponsor) && $sponsor->PaymentTerm == 'Monthly')
                  selected
                  @endif
                  >Monthly</option>
                  <option value="Quarterly" 
                  @if(isset($sponsor) && $sponsor->PaymentTerm == 'Quarterly')
                  selected
                  @endif
                  >Quarterly</option>
                
                  <option value="Yearly" 
                  @if(isset($sponsor) && $sponsor->PaymentTerm == 'Yearly')
                  selected
                  @endif
                  >Yearly</option>
                  <option value="Other" 
                  @if(isset($sponsor) && $sponsor->PaymentTerm == 'Other')
                  selected
                  @endif
                  >Other</option>
                  </select>
